...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        _mainWindow = new MainWindow(_searchService);
        _mainWindow.ViewModel.SetSyncInterval(_syncIntervalSeconds);
        ...
        try
        {
            _searchProvider = new SearchProvider(_searchService);
            _searchProvider.Start();
        }
        catch
        {
            // Search provider is non-critical, continue without it
        }
    ...
    private void SetupTrayIcon()
    {
        ...
        foreach (var (seconds, label) in SyncIntervalOptions)
        {
            var mi = new System.Windows.Controls.MenuItem
            {
                Header = label,
                IsCheckable = true,
                IsChecked = seconds == _syncIntervalSeconds,
                Tag = seconds
            };
            mi.Click += SyncIntervalMenuItem_Click;
            syncItem.Items.Add(mi);
        }
    ...
    private void SyncIntervalMenuItem_Click(object sender, RoutedEventArgs e)
    {
        if (sender is not System.Windows.Controls.MenuItem clicked) return;
        if (clicked.Tag is not int seconds) return;

        _syncIntervalSeconds = seconds;
        _mainWindow?.ViewModel.SetSyncInterval(seconds);
        SaveSettings();

        // Update checkmarks
        if (clicked.Parent is System.Windows.Controls.MenuItem parent)
        {
            foreach (var child in parent.Items)
            {
                if (child is System.Windows.Controls.MenuItem mi)
                    mi.IsChecked = mi.Tag is int s && s == seconds;
            }
        }
    }
    ...
    private void SaveHotkeyConfig() => SaveSettings();
    ...
    private void LoadHotkeyConfig()
    {
        ...
        try
        {
            ...
            // Try new format first (AppSettings)
            var settings = JsonSerializer.Deserialize<AppSettings>(json);
            if (settings?.Hotkey != null)
            {
                _hotkeyConfig = settings.Hotkey;
                _syncIntervalSeconds = settings.SyncIntervalSeconds > 0 ? settings.SyncIntervalSeconds : 60;
                return;
            }

            // Fall back to legacy format (HotkeyConfig only)
